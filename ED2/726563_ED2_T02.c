...
//
void imprimirNoIp(node_Btree_ip *no);

//
void listagemIp(node_Btree_ip *no, int nivel);

//
void listagemIs(node_Btree_is *no);

//
void imprimirNoIs(node_Btree_is *no);
...
void write_btree(void *salvar, int rrn, char ip){
	...
	if(ip=='p'){
		char aux[tamanho_registro_ip+1];
		...
		*(aux+3+(ordem_ip-1)*14) = ((node_Btree_ip *) salvar)->folha;
		...
		char aux[tamanho_registro_is+1];
...
void *read_btree(int rrn, char ip){
	...
	if(ip=='p'){
		...
		for(i=0; i<((node_Btree_ip *) no)->num_chaves; i++){
			...
			((node_Btree_ip *) no)->chave[i].pk[TAM_PRIMARY_KEY-1] = '\0';
			p = p + 10;
		...
		for(i=0, j=0; i<((node_Btree_is *) no)->num_chaves; i++, j=0){
			...
			((node_Btree_is *) no)->chave[i].pk[TAM_PRIMARY_KEY-1] = '\0';
			p = p + 10;
...
char *buscaIs(node_Btree_is *no, char *string, char caminho){
	...
	if(no->folha=='F'){
		if(i==no->num_chaves || strcmp(string, no->chave[i].string)!=0)
			...
			pk = (char *)malloc(TAM_PRIMARY_KEY);
			strcpy(pk, no->chave[i].pk);
		}
		libera_no((void *) no, 's');
		return(pk);
	}
	else if(i==no->num_chaves || strcmp(string, no->chave[i].string)!=0){
...
int alterar(Indice iprimary){
	...
	scanf("%s%*c", pk);
	if(iprimary.raiz==-1 || (rrn=buscaIp((node_Btree_ip *) read_btree(iprimary.raiz, 'p'), pk, 0))==-1){//caso o registro nao seja encontrado
...
void buscar(Indice iprimary,Indice idev){
	...
	if(op==1){
		...
		rrn = buscaIp((node_Btree_ip *) read_btree(iprimary.raiz, 'p'), pk, 1);
		printf("\n");
		if(rrn==-1)
			printf(REGISTRO_N_ENCONTRADO);
		else
		...
		char string[TAM_STRING_INDICE], aux[TAM_DESENVOLVEDORA];
		char *pk;
		fgets(aux, TAM_DESENVOLVEDORA-1, stdin);
		strcpy(string, aux);
		string[strlen(aux)-1] = '$';
		fgets(aux, TAM_DESENVOLVEDORA-1, stdin);
		strcat(string, aux);
		...
		pk = buscaIs((node_Btree_is *) read_btree(idev.raiz, 's'), string, 1);
		printf("\n");
		if(pk!=NULL){
			rrn = buscaIp((node_Btree_ip *) read_btree(iprimary.raiz, 'p'), pk, 0);
			exibir_registro(rrn);
			free(pk);
		}
		else
...
void listar(Indice iprimary,Indice idev){
	...
	if(op==1){
		if(iprimary.raiz!=-1)
			listagemIp((node_Btree_ip *) read_btree(iprimary.raiz, 'p'), 1);
		else
			printf(REGISTRO_N_ENCONTRADO);
	}
	else if(op==2){
		if(idev.raiz!=-1){
			node_Btree_is *no = (node_Btree_is *) read_btree(idev.raiz, 's');
			//printf("%c\n", no->folha);
			listagemIs(no);
		}
		else
...
void listagemIp(node_Btree_ip *no, int nivel){
	...
	if(no->folha!='F'){
		while(i<=no->num_chaves){
			listagemIp((node_Btree_ip *) read_btree(no->desc[i], 'p'), nivel+1);
			i++;
		}
	}
	libera_no((void *) no, 'p');
...
void listagemIs(node_Btree_is *no){
	int i=0, j;
	char *c, aux[TAM_STRING_INDICE];

	if(no->folha=='F'){
		while(i<no->num_chaves){
			strcpy(aux, no->chave[i].string);
			c = strtok(aux, "$");
			printf("%s", c);
			j = strlen(c);
			while(j<50){
		...
		while(i<no->num_chaves){
			listagemIs((node_Btree_is *) read_btree(no->desc[i], 's'));
			strcpy(aux, no->chave[i].string);
			c = strtok(aux, "$");
			printf("%s", c);
			j = strlen(c);
