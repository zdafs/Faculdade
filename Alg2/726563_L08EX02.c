/* ================================================================== *
    ...
    Lista 08 - Exercício 02 - Cadastro de Alunos
...
int carregar_arquivo(aluno alunos[MAX_ALUNO])
{
	...
	while(fscanf(ptr, "%d@%d@%d@", &(alunos[i].ra), &(alunos[i].ano), &(alunos[i].creditos))!=EOF)
	{
		j=0;
		while((c=fgetc(ptr))!='\n')
		{
...
void escrever_arquivo(aluno alunos[MAX_ALUNO], int numAlunos)
{
	...
	ptr=fopen(ARQUIVO, "w");
	for(i=0; i<numAlunos; i++)
		fprintf(ptr, "%d@%d@%d@%s\n", alunos[i].ra, alunos[i].ano, alunos[i].creditos, alunos[i].nome);
	fclose(ptr);
}